...
ANDROID_OUTPUTS_DIR="dohabits-android/build/outputs"
AVDMANAGER="${ANDROID_HOME}/cmdline-tools/latest/bin/avdmanager"
AVD_PREFIX="uhabitsTest"
EMULATOR="${ANDROID_HOME}/emulator/emulator"
GRADLE="./gradlew --stacktrace --quiet"
PACKAGE_NAME=com.dodo.dohabits
SDKMANAGER="${ANDROID_HOME}/cmdline-tools/latest/bin/sdkmanager"
VERSION=$(grep versionName dohabits-android/build.gradle.kts | sed -e 's/.*"\([^"]*\)".*/\1/g')
BOOT_TIMEOUT=360

if [ -z $VERSION ]; then
    echo "Could not parse app version from: dohabits-android/build.gradle.kts"
    exit 1
fi
...
core_build() {
    log_info "Building dohabits-core..."
    $GRADLE ktlintCheck || fail
    $GRADLE lintDebug || fail
    $GRADLE :dohabits-core:build || fail
}
...
android_test() {
    ...
    if [ -n "$RELEASE" ]; then
        ...
        $ADB install -r ${ANDROID_OUTPUTS_DIR}/apk/release/dohabits-android-release.apk || return 1
    else
        log_info "Installing debug APK..."
        $ADB install -t -r ${ANDROID_OUTPUTS_DIR}/apk/debug/dohabits-android-debug.apk || return 1
    fi
    log_info "Installing test APK..."
    $ADB install -r ${ANDROID_OUTPUTS_DIR}/apk/androidTest/debug/dohabits-android-debug-androidTest.apk || return 1
...
android_build() {
    log_info "Building dohabits-android..."
    ...
    rm -vf dohabits-android/build/*.apk

    if [ -n "$RELEASE" ]; then
        log_info "Building release APK..."
        $GRADLE updateTranslators
        $GRADLE :dohabits-android:assembleRelease
        cp -v \
            dohabits-android/build/outputs/apk/release/dohabits-android-release.apk \
            dohabits-android/build/loop-"$VERSION"-release.apk
    fi

    log_info "Building debug APK..."
    $GRADLE :dohabits-android:assembleDebug --stacktrace || fail
    cp -v \
        dohabits-android/build/outputs/apk/debug/dohabits-android-debug.apk \
        dohabits-android/build/loop-"$VERSION"-debug.apk

    log_info "Building instrumentation APK..."
    if [ -n "$RELEASE" ]; then
        $GRADLE :dohabits-android:assembleAndroidTest  \
...
android_accept_images() {
    ...
    rsync -av ${ANDROID_OUTPUTS_DIR}/test-screenshots/ dohabits-android/src/androidTest/assets/
}
...
_print_usage() {
    cat <<END
CI/CD script for Daily Loop Tracker.
...
clean() {
    rm -rfv dohabits-android/.gradle
    rm -rfv dohabits-android/android-pickers/build
    rm -rfv dohabits-android/build
    rm -rfv dohabits-android/dohabits-android/build
    rm -rfv dohabits-core-legacy/.gradle
    rm -rfv dohabits-core-legacy/build
    rm -rfv dohabits-core/.gradle
    rm -rfv dohabits-core/build
